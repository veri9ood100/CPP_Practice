...
SinglyLinkedList list;  // Assuming SinglyLinkedList has a default constructor.

void putItems(const vector<string>& keys, bool debugFlag, bool sortFlag) {
  for (const auto& key : keys) {
    // cout << "Putting " << key <<"\n";
    list.put(key, "value_" + key.substr(4),
             sortFlag);  // assuming that key is in the form "key_X"
    if (debugFlag) {
      cout << "Key: " << key << ", Value: "
           << "value_" + key.substr(4) << "\n";
    }
...
int main(int argc, char** argv) {
  ...
  elapsedPut =
      measureExecutionTime([&]() { putItems(keys, debugFlag, sortFlag); });
