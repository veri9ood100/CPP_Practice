...
void putItems(const vector<string>& keys, bool debugFlag) {
  for (const auto& key : keys) {
    // cout << "Putting " << key <<"\n";
    HTS.put(key, "value_" + key.substr(4));  // assuming that key is in the form "key_X"
           if (debugFlag) {
             cout << "Key: " << key << ", Value: "
                  << "value_" + key.substr(4) << "\n";
           }
...
void getItems(const vector<string>& keys, bool debugFlag) {
  for (const auto& key : keys) {
    ...
    // cout << "getting " << key <<"\n";
    string value = HTS.get(key);

    if (debugFlag) {
      cout << "Value: " <<HTS.get(key) << "\n";
    }
...
int main(int argc, char** argv) {
  ...
  elapsedPut =
      measureExecutionTime([&]() { putItems(keys, debugFlag); });
  cout << "Time taken to put " << NR_TESTS << " items: " << elapsedPut
       << " seconds.\n";

  if (randomFlag) keys = createKeys(randomFlag);
  elapsedGet = measureExecutionTime([&]() { getItems(keys, debugFlag); });
  cout << "Time taken to get " << NR_TESTS << " items: " << elapsedGet
       << " seconds.\n";

  if (randomFlag) keys = createKeys(randomFlag);
  elapsedRemove = measureExecutionTime([&]() { removeItems(keys, debugFlag); });
  cout << "Time taken to remove " << NR_TESTS << " items: " << elapsedRemove
       << " seconds.\n";
